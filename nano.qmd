...
At the bottom of the window, there is a list of the most basic command shortcuts to use with the nano editor. All commands are prefixed with either `^` or `M` characters. The caret symbol (`^`) represents the `Ctrl` key. For example, the `^J` commands mean to press the `Ctrl` and `J` keys at the same time. The letter `M` represents the `Alt` or `Option` key. You can get a list of all commands by typing `Ctrl+g`.
